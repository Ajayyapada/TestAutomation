...
//       	  }
    ...
    public String[][] ExecuteQuery(String Query, String SQLDBServer, String SqlDBInstance, String SqlDBName, String UserName, String Password){
        ...
        try{
			...
			Driver.CUtil.WriteResults("Query = " + Query);
            ...
            */ con= this.getConnection(SQLDBServer,SqlDBInstance,SqlDBName,UserName,Password);
             if(con!=null){
                  Statement sqlstmt = con.createStatement();
                  sqlstmt.setQueryTimeout(300); // Updated by Sonu on 4-4-16 -  setting the max query time out to 5 min
                  rs = sqlstmt.executeQuery(Query);
                  //Message,functionname,status=> Done,Pass,Fail,Warning,N/A Mar 26, 2014 8:13:39 PM
                  Driver.CUtil.WriteResults("Query executed successfully");
                  ColCount = rs.getMetaData().getColumnCount();
                  String Data = "";
                  while(rs.next()){
                       DBRValue += rs.getRow();
					...
					Driver.CUtil.WriteResults("Row Data = " + Data);
					Data = "";
				 ...
				 if(IsDBReturnedRow){//If condition added:Madhu:25/04/2014
                  Driver.CUtil.WriteResults(":: All Rows printed ::");
				 }
				 else
				 {
					 Driver.CUtil.WriteResults(":: No Rows printed ::"); 
				 }
                  String[] Rows = DBRValue.split("~");
                  String[] Col1 = Rows[0].split("`");
                  int RLen = Rows.length;
            	 ...
            	 Driver.CUtil.WriteResults("Error: No active Connection");
    ...
    /**
     * this function will close the DB connection
     */
...
/**
 * This function will print line in console 
 * @param message
 */
